...
*/
    ...
    void CheckForUserInput()
    {
        ...
        // Get the input from the right VR controller's trigger
        InputDevice rightHand = InputDevices.GetDeviceAtXRNode(XRNode.RightHand);
        if (rightHand.TryGetFeatureValue(triggerFeature, out float triggerValue))
        {
            // Consider the trigger pressed if the value exceeds a certain threshold
            isTriggerPressed = triggerValue > 0.1f;
        }
        ...
        if (type == WeaponType.Raycast)
        {
            if (fireTimer >= actualROF && burstCounter < burstRate && canFire)
            {
                if (isTriggerPressed)
                {
                    ...
                    {
                        Fire();
                        VibrateController(rightHand); // Trigger vibration
                    }
                    else if (heat < maxWarmup) // Otherwise just add to the warmup until the user lets go of the button
                    {
                        heat += Time.deltaTime;
                ...
                if (warmup && !isTriggerPressed) // Trigger released
                {
                    if (allowCancel && Input.GetButton("Cancel"))
                    {
                        heat = 0.0f;
        ...
        if (type == WeaponType.Beam)
        {
            if (isTriggerPressed && beamHeat <= maxBeamHeat && !coolingDown)
            {
                Beam();
                VibrateController(rightHand); // Trigger vibration
            }
    ...
    private void VibrateController(InputDevice device)
    {
        // Set the intensity and duration of the vibration
        float intensity = 0.5f; // Intensity of the vibration (0.0 to 1.0)
        float duration = 0.1f;  // Duration of the vibration in seconds
	...
	public void Launch()
	{
		...
		// Play the gunshot sound
		GetComponent<AudioSource>().PlayOneShot(fireSound);
		OrbShooter orbShooter = GetComponentInChildren<OrbShooter>();
		if(orbShooter != null)
		{
			orbShooter.ChangeOrb(currentAmmo,ammoCapacity);
		}
	}
